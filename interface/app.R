...
parse_num_vec <- function(txt){nums<-gsub("\\s+","",txt); if(nzchar(nums)) as.numeric(strsplit(nums,",")[[1]]) else numeric(0)}
parse_edges <- function(txt){
  ...
  d <- do.call(rbind,lapply(parts,function(p){ if(length(p)<2) return(c(NA,NA,NA)); c(p[1],p[2], ifelse(length(p)>=3,p[3],"1")) }))
...
server <- function(input, output, session){
  ...
  observeEvent(input$uf_run,{
    ...
    ops <- gsub("\\s+","",input$uf_ops); if(nzchar(ops)){
      for(p in strsplit(ops,",")[[1]]){ ab<-strsplit(p,"-")[[1]]; if(length(ab)==2) uf$union(as.integer(ab[1]), as.integer(ab[2])) }
    }
    comp <- data.frame(node=1:n, parent=sapply(1:n, uf$find))
    output$uf_tbl <- renderDT(datatable(comp, options=list(pageLength=10)))
  })

  output$dp_grid <- renderDT(datatable(as.data.frame(matrix(sample(1:9,9,TRUE),3,3)), options=list(dom='t'), rownames=FALSE))
  observeEvent(input$dp_gen,{
    m<-as.integer(input$dp_m); n<-as.integer(input$dp_n)
    output$dp_grid <- renderDT(datatable(as.data.frame(matrix(sample(1:9,m*n,TRUE),m,n)), options=list(dom='t'), rownames=FALSE))
  })
  observeEvent(input$dp_run,{
    tbl <- DT::datatableProxy("dp_grid") # just to ensure the widget exists
    # regenerate the grid like above (simple demo)
    g <- matrix(sample(1:9, as.integer(input$dp_m)*as.integer(input$dp_n), TRUE),
                as.integer(input$dp_m), as.integer(input$dp_n))
    if (exists("min_path_sum")) {
      output$dp_out <- renderText(paste("Min path sum:", min_path_sum(g)))
    } else output$dp_out <- renderText("min_path_sum() not found")
  })
  ...
  observeEvent(input$ts_run,{
    req(exists("topological_sort")); edges <- parse_edges(input$ts_edges)
    ord <- topological_sort(edges); output$ts_out <- renderText(if(is.null(ord)) "Cycle detected" else paste(ord, collapse=" -> "))
  })
}
